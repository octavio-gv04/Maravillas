...
import { card, btn, btnGhost, field, select, textarea, sectionHead, empty, badge } from '../ui.js';
...
export function render(container) {
  ...
  const formCard = () => {
    ...
    return card(`
    <h2 class="font-semibold mb-3">${editId ? '✏️ Editar ingreso' : '➕ Nuevo ingreso'}</h2>
  ...
  const tableCard = () => {
    ...
    return card(`
      ${sectionHead(`Ingresos (${list.length})`,
        `<input id="ing-search" class="field !w-56" placeholder="🔍 Buscar..." value="${esc(query)}" />`)}
            ...
            ${list.map((x) => `
              <tr class="border-b border-gray-100 dark:border-gray-700/50">
                ...
                <td class="text-right whitespace-nowrap">
                  <button data-print="${x.id}" class="px-2 hover:text-brand" title="Imprimir recibo">🖨️</button>
